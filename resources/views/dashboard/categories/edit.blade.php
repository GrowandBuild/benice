    ...
    <div class="bg-gradient-to-r from-blue-600 via-cyan-500 to-teal-500 shadow-lg">
        <div class="max-w-7xl mx-auto px-6 py-8">
            <div class="flex justify-between items-center">
                <div>
                    <h1 class="text-4xl font-bold text-white mb-2">✏️ Editar Categoria</h1>
                    <p class="text-blue-100 text-lg">Atualize as informações da categoria</p>
                </div>
    ...
    <div class="max-w-3xl mx-auto px-6 py-12">
        <div class="bg-white rounded-3xl shadow-2xl overflow-hidden border border-blue-100">
            <div class="bg-gradient-to-r from-blue-50 to-cyan-50 px-8 py-6 border-b border-blue-200">
                <h2 class="text-2xl font-bold text-blue-800">📝 Editar Categoria: {{ $category->name }}</h2>
            </div>
            
            <form action="{{ route('dashboard.categories.update', $category) }}" method="POST" class="p-8">
                @csrf
                @method('PUT')
                ...
                <div class="mb-8">
                    ...
                    <p class="mt-2 text-blue-600 text-sm">O slug é usado na URL da categoria</p>
                    @error('slug')
                        <p class="mt-2 text-red-600 text-sm">{{ $message }}</p>
                    @enderror
                </div>
